...
module.exports = db => {
  ...
  //a db model is a definition of what a particular data type
  //is going to look like

  //db.createModel takes in 3 parameters (1 is optional)
  // createModel(modelName, modelSchema, modelOptions)
  //modelName - the name of the db table/collection
  //the records will be stored in

  //modelSchema - the shape/definition of what the data will look like

  //modelOptions - alternate settings to change the behaviour of how our
  //data is validated, saved, or queried
  ...
  User.define("generatePassword", function() {
    return bcrypt
      ...
      .then(hash => Object.assign(this, { password: hash })) //update user with the hash as password
      .catch(err => err); //catch any thrown errors and return them
  });
  ...
  // defines a method to transform the user data into a secure access token
  //to our API
  ...
  User.pre("save", function(next) {
    User.filter({ email: this.email }).then(users => {
      //check if users array that have the same email address is longer then 0
      //if longer then 0, then proceed with an error
      if (users.length > 0) {
